...
* **Управление жизненным циклом** — формальный конечный автомат на HFSM2: uninitialized → initializing → operation → stopping → settled (с отдельным путём ошибки failed)
...
* https://cmake.org[CMake] 3.28+ с Ninja — основная (authoritative) система сборки
* https://vcpkg.io[Vcpkg] (manifest mode) — менеджер пакетов, используется через `VCPKG_ROOT`
* Опционально, https://xmake.io[Xmake] 2.8+ — вторичный workflow с smoke-покрытием только для
  Linux (см. `docs/modules/ROOT/pages/architecture/build-systems.adoc`)
...
Зависимости объявлены в `vcpkg.json` (manifest mode) и устанавливаются автоматически
через vcpkg toolchain-файл CMake. Базовый набор: `reflectcpp`, `folly`, `spdlog`,
`hfsm2`; опциональные vcpkg-фичи добавляют HTTP-транспорты `drogon`/`crow`,
HTTP-клиент (`openssl`), OpenTelemetry (`otel`) и тестовый набор на `gtest`
(`tests`) — см. переменную кэша `VCPKG_MANIFEST_FEATURES` для каждого пресета в
`CMakePresets.json`.

=== Сборка (CMake — основной способ)
...
Другие пресеты: `linux-release`, `linux-minimal` (без тестов и опциональных
транспортов), `linux-all-features` (drogon/crow/otel/http-client включены),
`linux-asan`/`linux-tsan`/`linux-ubsan` (санитайзеры), `clang-debug`/`gcc-debug`.
Полная матрица ответственности CMake/Xmake — в
`docs/modules/ROOT/pages/architecture/build-systems.adoc`: CMake отвечает за
install-правила, CPack и CI; Xmake — только Linux smoke-покрытие.

=== Сборка (Xmake — вторичный способ)
...
package "Core Server" {
  [McpServer] --> [McpRequestHandler] : управляет
  [McpRequestHandler] --> [ToolRegistry] : использует
  [SessionManager] --> [ServerSession] : создаёт на соединение
}

package "Protocol" {
  [ServerSession] --> [JsonRpcCodec] : использует
  [ServerSession] --> [ProtocolDialect] : использует
}

Client --> StdioTransport : stdin/stdout, HTTP
StdioTransport --> McpRequestHandler : JSON-RPC
@enduml
----

`McpRequestHandler`/`McpServer` — пара, на которой сейчас работают встроенные
примеры. `ServerSession`/`SessionManager` — более новый, основанный на
конечном автомате путь (см. <<Жизненный цикл сессии>>), на который
постепенно переводится обработка запросов.

=== Жизненный цикл сессии

`ServerSession` (реализация сессии на основе конечного автомата) моделирует
жизненный цикл как формальный HFSM2-автомат:

1. **Uninitialized** → ожидание `initialize`; согласовывает `ProtocolDialect`
   по запрошенной клиентом `protocolVersion` через `JsonRpcCodec`
2. **Initializing** → `initialize` принят, ожидание
   `notifications/initialized` (либо таймаут, переводящий в **Failed**)
3. **Operation** → активная обработка запросов (`tools/list`, `tools/call`,
   `ping`), с отменой отдельных запросов через `folly::CancellationSource`
4. **Stopping** → штатное завершение работы: ожидание завершения активных
   запросов до истечения settle-таймаута
5. **Settled** → терминальное состояние

Подробный дизайн — в
`docs/modules/ROOT/pages/architecture/server-session-design.adoc`, общая
архитектура и roadmap — в
`docs/modules/ROOT/pages/architecture/index.adoc`.

== Тестирование

Запустите полный набор тестов CTest (unit + интеграционные) через пресет CMake:
...
Проект также включает примеры серверов, которые можно использовать как
ручные/интеграционные smoke-тесты:
...
4. Соберите и протестируйте локально: `cmake --build --preset linux-debug && ctest --preset linux-debug`
5. Отправьте PR — CI прогонит полную матрицу CMake (gcc/clang, debug/release, all-features, санитайзеры) относительно `devel`
...
Каталог `docs/` (Antora) содержит основную архитектурную документацию и roadmap:

* `docs/modules/ROOT/pages/architecture/index.adoc` — архитектура SDK, граф
  модулей и этапы миграции
* `docs/modules/ROOT/pages/architecture/build-systems.adoc` — матрица
  ответственности CMake/Xmake
* `docs/modules/ROOT/pages/architecture/server-session-design.adoc` —
  конечный автомат `ServerSession`
* `docs/modules/ROOT/pages/architecture/epic-*-implementation-plan.adoc` —
  планы реализации по эпикам и задачам
