...
* **Lifecycle management** — full implementation of stages: uninitialized → initialized → operation → shutdown
...
* Compiler with full C++20 support (GCC 11+, Clang 13+, MSVC 2022+, AppleClang 14+)
* https://cmake.org[CMake] 3.28+ (primary build system)
* https://ninja-build.org[Ninja] — build tool (required by CMake presets)
* https://vcpkg.io[Vcpkg] — package manager (set `VCPKG_ROOT` environment variable)
* Optional: https://xmake.io[Xmake] 2.8+ (secondary build system)

== Installation and Build

PhoenixMcp uses **CMake as its primary build system**. Xmake is supported as a secondary frontend for development smoke checks and does not provide package export or install targets.
...
cd PhoenixMcp
export VCPKG_ROOT=/path/to/vcpkg   # required for dependency resolution
----

=== CMake Build (primary)

Debug build with minimal features (stdio-only):
...
Debug build with all available features (requires HTTP dependencies):
...
=== Package Install and Consumer Workflow
...
A downstream CMake project can then find the SDK via `find_package`:
...
=== Xmake Build (secondary)

Xmake provides an alternative development build for Linux smoke checks. It does not support package export or install.
...
HTTP backends and telemetry are controlled by environment variables:

[source,bash]
----
PHOENIX_MCP_WITH_DROGON=1 xmake f -m debug -y   # enable Drogon transport
PHOENIX_MCP_WITH_OTEL=1 xmake f -m debug -y      # enable OpenTelemetry
----
...
[source,bash]
----
xmake run create_server          # stdio-based MCP server
xmake run using_tool_registry    # standalone tool registry demo
----
...
2. **Initialized** → Initialization received, waiting for `notifications/initialized` notification
3. **Operation** → Active request processing (`tools/list`, `tools/call`, `ping`)
4. **Shutdown** → Server termination
...
The project includes CTest-discoverable unit tests and integration smoke checks.
...
* JSON-RPC message parsing (request, notification, response, error)
* Error model serialization and round-trip
* Public header compilation checks
* Installed package consumer verification
...
3. Apply formatting: `rtk run 'clang-format -i path/to/file.cc path/to/file.h'`
4. Build and test: `rtk run 'cmake --preset linux-debug && cmake --build --preset linux-debug && ctest --preset linux-debug --output-on-failure'`
...
* https://cmake.org[CMake Documentation]
